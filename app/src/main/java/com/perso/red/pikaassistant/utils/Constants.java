package com.perso.red.pikaassistant.utils;

/**
 * Created by pierr on 23/08/2016.
 */

public class Constants {

    public static final String FILE_POKEMON = "pokemon.json";
    public static final String FILE_POKEMON_EN = "pokemon_en.json";
    public static final String FILE_POKEMON_FR = "pokemon_fr.json";
    public static final String FILE_POKEMON_DE = "pokemon_de.json";
    public static final String FILE_POKEMON_JA = "pokemon_ja.json";
    public static final String FILE_POKEMON_RU = "pokemon_ru.json";
    public static final String FILE_DUST_TO_LVL = "dust_to_level.json";
    public static final String FILE_CPM_POKEMON = "pokemon_cpm.json";
    public static final String FILE_MAX_CP_POKEMON = "pokemon_max_cp.json";
    public static final String FILE_MOVE_SETS = "move_sets.json";
    public static final String FILE_EGG_G1 = "file:///android_asset/eggs_g1.png";
    public static final String FILE_EGG_G2 = "file:///android_asset/eggs_g2.jpg";
    public static final String FILE_BUDDIES_1_KM = "file:///android_asset/buddies_1km.png";
    public static final String FILE_BUDDIES_3_KM = "file:///android_asset/buddies_3km.png";
    public static final String FILE_BUDDIES_5_KM = "file:///android_asset/buddies_5km.png";
    public static final String FILE_BUDDIES_UNKNOWN_KM = "file:///android_asset/buddies_unknown_km.png";

    public static final String LANGUAGE_FR = "fr";
    public static final String LANGUAGE_EN = "en";
    public static final String LANGUAGE_DE = "de";
    public static final String LANGUAGE_JA = "ja";
    public static final String LANGUAGE_RU = "ru";

    public static final String POKEMON_TYPE_NORMAL = "NORMAL";
    public static final String POKEMON_TYPE_FIRE = "FIRE";
    public static final String POKEMON_TYPE_WATER = "WATER";
    public static final String POKEMON_TYPE_ELECTRIC = "ELECTRIC";
    public static final String POKEMON_TYPE_GRASS = "GRASS";
    public static final String POKEMON_TYPE_ICE = "ICE";
    public static final String POKEMON_TYPE_FIGHTING = "FIGHTING";
    public static final String POKEMON_TYPE_POISON = "POISON";
    public static final String POKEMON_TYPE_GROUND = "GROUND";
    public static final String POKEMON_TYPE_FLYING = "FLYING";
    public static final String POKEMON_TYPE_PSYCHIC = "PSYCHIC";
    public static final String POKEMON_TYPE_BUG = "BUG";
    public static final String POKEMON_TYPE_ROCK = "ROCK";
    public static final String POKEMON_TYPE_GHOST = "GHOST";
    public static final String POKEMON_TYPE_DRAGON = "DRAGON";
    public static final String POKEMON_TYPE_DARK = "DARK";
    public static final String POKEMON_TYPE_STEEL = "STEEL";
    public static final String POKEMON_TYPE_FAIRY = "FAIRY";

    public static final double[] CpM = {0.0939999967813492, 0.135137432089339, 0.166397869586945, 0.192650913155325, 0.215732470154762, 0.236572651424822, 0.255720049142838, 0.273530372106572, 0.290249884128571, 0.306057381389863
            , 0.321087598800659, 0.335445031996451, 0.349212676286697, 0.362457736609939, 0.375235587358475, 0.387592407713878, 0.399567276239395, 0.4111935532161, 0.422500014305115, 0.432926420512509, 0.443107545375824
            , 0.453059948165049, 0.46279838681221, 0.472336085311278, 0.481684952974319, 0.490855807179549, 0.499858438968658, 0.5087017489616, 0.517393946647644, 0.525942516110322, 0.534354329109192, 0.542635753803599
            , 0.550792694091797, 0.558830584490385, 0.566754519939423, 0.57456912814537, 0.582278907299042, 0.589887907888945, 0.597400009632111, 0.604823648665171, 0.61215728521347, 0.619404107958234, 0.626567125320435
            , 0.633649178748576, 0.6406529545784, 0.647580971386554, 0.654435634613037, 0.661219265805859, 0.667934000492096, 0.674581885647492, 0.681164920330048, 0.687684901255373, 0.694143652915955, 0.700542901033063
            , 0.706884205341339, 0.713169074873823, 0.719399094581604, 0.725575586915154, 0.731700003147125, 0.734741038550429, 0.737769484519958, 0.740785579737136, 0.743789434432983, 0.746781197247765, 0.749761044979095
            , 0.752729099732281, 0.75568550825119, 0.758630370209851, 0.761563837528229, 0.76448604959218, 0.767397165298462, 0.770297293677362, 0.773186504840851, 0.776064947064992, 0.778932750225067, 0.781790050767666
            , 0.784636974334717, 0.787473608513275, 0.790300011634827};

    public static final int CALCULATOR_MODE_ARC = 0;
    public static final int CALCULATOR_MODE_DUST = 1;

    public static final int VIEW_NONE = -1;
    public static final int VIEW_MENU = 0;
    public static final int VIEW_POKEMON = 1;
    public static final int VIEW_EGGS = 2;
    public static final int VIEW_IV_CALC = 3;
    public static final int VIEW_EVO_CALC = 4;
    public static final int VIEW_XP_CALC = 5;
    public static final int VIEW_APPRAISAL = 6;
    public static final int VIEW_BUDDIES = 7;

    public static final String LAUNCH_MODE = "launch mode";
    public static final String VIEW_SELECTED = "views selected";
    public static final String VIEW_SELECTED_IV_CALC = "iv calculator selected";
    public static final String VIEW_SELECTED_XP_CALC = "xp calculator selected";
    public static final String VIEW_SELECTED_EVO_CALC = "evo calculator selected";
    public static final String VIEW_SELECTED_EGGS = "eggs selected";
    public static final String VIEW_SELECTED_BUDDIES = "buddies selected";
    public static final String VIEW_SELECTED_APPRAISAL = "appraisal selected";

}
